...
def bottleneck4AIEs():
    with mlir_mod_ctx() as ctx:
        ...
        def deviceBody():
            ...
            ShimTileC = tile(1, 0)
            MemTileC = tile(1, 1)
            ComputeTile2C = tile(1, 2)
            ComputeTile3C = tile(1, 3)
            ComputeTile4C = tile(1, 4)
            ComputeTile5C = tile(1, 5)

            # runtime parameters
            rtpComputeTile2C = Buffer(ComputeTile2C, [16], T.i32(), "rtpComputeTile2C")
            rtpComputeTile3C = Buffer(ComputeTile3C, [16], T.i32(), "rtpComputeTile3C")
            rtpComputeTile4C = Buffer(ComputeTile4C, [16], T.i32(), "rtpComputeTile4C")
            rtpComputeTile5C = Buffer(ComputeTile5C, [16], T.i32(), "rtpComputeTile5C")
            ...
            of_wts_buf_00 = object_fifo("wts_buf_00", MemTile, [ComputeTile2, ComputeTile2C], 1, weightsLayer1_ty)
            wts_buf_01 = object_fifo("wts_buf_01", MemTile, [ComputeTile3, ComputeTile5, ComputeTile3C, ComputeTile5C],1, weightsLayer2_ty)
            wts_buf_02 = object_fifo("wts_buf_02", MemTile, [ComputeTile4, ComputeTile4C], 1, weightsLayer3_ty)
            ...
            # ===============
            # input tensor (to L1 and MemTile, which is later used by L3 skip)
            of_inOF_act_L3L2C = object_fifo("inOF_act_L3L2C", ShimTileC, [ComputeTile2C, MemTileC], [2, 2, 4], tensorLayer1In_ty)
            of_skip_bufC = object_fifo("skip_bufC", MemTileC, ComputeTile4C, 2, tensorLayer1In_ty)
            object_fifo_link(of_inOF_act_L3L2C, of_skip_bufC)


            # activation tensor (brocast from 1x1 to 2 3x3)
            of_act_2_3_5C = object_fifo("act_2_3_5C", ComputeTile2C, [ComputeTile3C, ComputeTile5C], [2, 4, 4], tensorLayer1Out_ty)  # 1x1 -> 3x3
            act_3_4C = object_fifo("act_3_4C", ComputeTile3C, ComputeTile4C, 2, tensorLayer2Out_ty)  # 3x3 -> 1x1
            act_5_4C = object_fifo("act_5_4C", ComputeTile5C, ComputeTile4C, 2, tensorLayer2Out_ty)  # 3x3 -> 1x1

            # output tensor
            outOFL2L3C = object_fifo("outOFL2L3C", ComputeTile4C, ShimTileC, 2, tensorLayer3Out_ty)

            # 1x1 conv2d
            @core(ComputeTile2C, "conv2dk1.o")
            def core_body():
                for _ in for_(sys.maxsize):
                    # acquire weights once
                    element0Weights = of_wts_buf_00.acquire(ObjectFifoPort.Consume, 1)
                    scale = memref.load(rtpComputeTile2C, [0])
                    for _ in for_(tensorInH):
                        element0ActivactionsIn = of_inOF_act_L3L2C.acquire(ObjectFifoPort.Consume, 1)
                        element0ActivactionsOut = of_act_2_3_5C.acquire(ObjectFifoPort.Produce, 1)
                        res = call(conv2dk1,[
                                element0ActivactionsIn, element0Weights, element0ActivactionsOut,
                                tensorInW, tensorL1InC, tensorL1OutC, scale])

                        objectfifo_release(ObjectFifoPort.Consume, "inOF_act_L3L2C", 1)
                        objectfifo_release(ObjectFifoPort.Produce, "act_2_3_5C", 1)
                        yield_([])
                    objectfifo_release(ObjectFifoPort.Consume, "wts_buf_00", 1)
                    yield_([])

            # 3x3 conv2d that produces half of the output channels
            @core(ComputeTile3C, "conv2dk3.o")
            def core_body():
                scale = 11
                for _ in for_(sys.maxsize):
                    element0Weights = wts_buf_01.acquire(ObjectFifoPort.Consume, 1)
                    ...
                    # pre-amble: top row
                    elementActivactionsIn = of_act_2_3_5C.acquire(ObjectFifoPort.Consume, 2)
                    element0ActivactionsOut = act_3_4C.acquire(ObjectFifoPort.Produce, 1)
                    res = call(conv2dk3,
                        [ elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1],
                            element0Weights, element0ActivactionsOut,
                            tensorInW, tensorL2InC, tensorL2OutC,
                            3, 3, 0, scale, 0])
                    objectfifo_release(ObjectFifoPort.Produce, "act_3_4C", 1)

                    # middle
                    for _ in for_(tensorInH - 2):
                        elementActivactionsIn = of_act_2_3_5C.acquire(ObjectFifoPort.Consume, 3)
                        element0ActivactionsOut = act_3_4C.acquire(ObjectFifoPort.Produce, 1)
                        res = call(conv2dk3,[
                                elementActivactionsIn[0], elementActivactionsIn[1], elementActivactionsIn[2],
                                element0Weights, element0ActivactionsOut,
                        ...
                        objectfifo_release(ObjectFifoPort.Consume, "act_2_3_5C", 1)
                        objectfifo_release(ObjectFifoPort.Produce, "act_3_4C", 1)
                        yield_([])
                    ...
                    objectfifo_release(ObjectFifoPort.Consume, "act_2_3_5C", 2)
                    objectfifo_release(ObjectFifoPort.Produce, "act_3_4C", 1)
                    objectfifo_release(ObjectFifoPort.Consume, "wts_buf_01", 1)
                    yield_([])

            # 3x3 conv2d that produces the other half of the output channels
            @core(ComputeTile5C, "conv2dk3.o")
            def core_body():
                scale = 11
                for _ in for_(sys.maxsize):
                    ...
                    # pre-amble: top row
                    elementActivactionsIn = of_act_2_3_5C.acquire(ObjectFifoPort.Consume, 2)
                    element0ActivactionsOut = act_5_4C.acquire(ObjectFifoPort.Produce, 1)
                    res = call(conv2dk3, [
                            elementActivactionsIn[0], elementActivactionsIn[0], elementActivactionsIn[1],
                            element0Weights, element0ActivactionsOut,
                            tensorInW, tensorL2InC, tensorL2OutC, 3, 3, 0, scale,
                    ...
                    objectfifo_release(ObjectFifoPort.Produce, "act_5_4C", 1)
            ...
            # # 1x1 conv2d and add skip
            @core(ComputeTile4C, "conv2dk1_skip.o")
            def core_body():
                for _ in for_(sys.maxsize):

                    # acquire weights and rtps once
                    element0Weights = wts_buf_02.acquire(ObjectFifoPort.Consume, 1)
                    scale = memref.load(rtpComputeTile4C, [0])
                    skipScale = memref.load(rtpComputeTile4C, [1])

                    for _ in for_(tensorInH):
                        element0ActivactionsIn = act_3_4C.acquire(ObjectFifoPort.Consume, 1)
                        element1ActivactionsIn = act_5_4C.acquire(ObjectFifoPort.Consume, 1)
                        elementSkipsIn = of_skip_bufC.acquire(ObjectFifoPort.Consume, 1)
                        elementActivactionsOut = outOFL2L3C.acquire(ObjectFifoPort.Produce, 1)
                        call(conv2dk1_skip, [element0ActivactionsIn,element1ActivactionsIn,
                                element0Weights, elementActivactionsOut, elementSkipsIn,
                                tensorInW, tensorL3InC, tensorL3OutC, scale, skipScale])
                        objectfifo_release(ObjectFifoPort.Produce, "outOFL2L3C", 1)
                        objectfifo_release(ObjectFifoPort.Consume, "act_3_4C", 1)
                        objectfifo_release(ObjectFifoPort.Consume, "act_5_4C", 1)
                        objectfifo_release(ObjectFifoPort.Consume, "skip_bufC", 1)
                        yield_([])
                    objectfifo_release(ObjectFifoPort.Consume, "wts_buf_02", 1)
                    yield_([])
            # ===============
            ...
            def sequence(inputFromL3, weightsFromL3, outputToL3):
                ...
                NpuWriteRTPOp("rtpComputeTile2C", index=0, value=1)  # scale
                NpuWriteRTPOp("rtpComputeTile3C", index=0, value=1)  # scale
                NpuWriteRTPOp("rtpComputeTile5C", index=0, value=1)  # scale
                NpuWriteRTPOp("rtpComputeTile4C", index=0, value=1)  # scale: 
                NpuWriteRTPOp("rtpComputeTile4C", index=1, value=0)  # skip_scale
                ...
                for i in range(1):
                    ...
                    npu_dma_memcpy_nd(metadata="inOF_act_L3L2C", 
                        bd_id=3, mem=inputFromL3, sizes=[1, 1, 1, activationsIn], offsets=[0, 0, 0, (2*i+1) * activationsIn])
                    npu_dma_memcpy_nd(metadata="outOFL2L3C",
                        bd_id=4, mem=outputToL3, sizes=[1, 1, 1, acitivationsOut], offsets=[0, 0, 0, (2*i+1) * acitivationsOut])
